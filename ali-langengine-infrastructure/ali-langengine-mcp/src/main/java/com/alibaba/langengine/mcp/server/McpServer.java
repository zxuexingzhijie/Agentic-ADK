...
import com.alibaba.langengine.mcp.spec.schema.resources.ResourceTemplate;
...
public interface McpServer {
	...
	public static class Builder {
		...
		private final static Implementation DEFAULT_SERVER_INFO = new Implementation("mcp-server",
				"1.0.0");
		...
		private List<Consumer<List<Root>>> rootsChangeConsumers = new ArrayList<>();
		 ...
		 */
		public Builder tool(Tool tool, Function<Map<String, Object>, CallToolResult> handler) {
		...
		public Builder prompts(List<PromptRegistration> prompts) {
			for (PromptRegistration prompt : prompts) {
				this.prompts.put(prompt.getPropmpt().getName(), prompt);
			}
			return this;
		}
		 ...
		 */
		public Builder rootsChangeConsumer(Consumer<List<Root>> consumer) {
		 ...
		 */
		public Builder rootsChangeConsumers(List<Consumer<List<Root>>> consumers) {
		 ...
		 * @throws IllegalArgumentException if consumers is null
		 */
		public Builder rootsChangeConsumers(Consumer<List<Root>>... consumers) {
			for (Consumer<List<Root>> consumer : consumers) {
				this.rootsChangeConsumers.add(consumer);
			}
			return this;
		}
