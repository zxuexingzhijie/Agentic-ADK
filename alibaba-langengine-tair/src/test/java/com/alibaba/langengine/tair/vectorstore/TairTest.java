...
@ContextConfiguration(classes = {BeanConfiguration.class, TairConfig.class})
public class TairTest {
    ...
    public void test_similaritySearch() {
        List<Document> result = tair.similaritySearch("接口taobao.traderates.get没有获取评价信息", 1);
        System.out.println(JSON.toJSONString(result));

        result = tair.similaritySearch("alibaba.ascp.logistics.offline.send报错是什么原因？", 1);
        System.out.println(JSON.toJSONString(result));

        result =tair.similaritySearch("alibaba.item.edit.schema.get", 1);
        System.out.println(JSON.toJSONString(result));
    }
