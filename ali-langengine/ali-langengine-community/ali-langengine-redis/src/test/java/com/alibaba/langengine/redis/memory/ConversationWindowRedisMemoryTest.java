 ...
 * 功能说明：
 */
public class ConversationWindowRedisMemoryTest {

    @Test
    public void testRdbWindowCount() {
        // 需要本地启动一个redis服务器
        ...
        try {
            ...
            ConversationWindowRedisMemory windowRedisMemory = new ConversationWindowRedisMemory(redisCache, "test");
            windowRedisMemory.setHistoryCount(2);

            ConversationChain conversation = new ConversationChain();
            ChatOpenAI llm = new ChatOpenAI();
            conversation.setLlm(llm);
            conversation.setVerbose(false);
