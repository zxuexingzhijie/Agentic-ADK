...
@ContextConfiguration(classes = {BeanConfiguration.class, PolarPostgresConfig.class})
public class PolarPostgresDBTest {

    @Resource
    private PolarPostgresDB polarPostgresDB;

    @Test
    public void test_similaritySearch() {
        List<Document> result = polarPostgresDB.similaritySearch("接口taobao.traderates.get没有获取评价信息", 1);
        System.out.println(JSON.toJSONString(result));

        result = polarPostgresDB.similaritySearch("alibaba.ascp.logistics.offline.send报错是什么原因？", 1);
        System.out.println(JSON.toJSONString(result));

        result =polarPostgresDB.similaritySearch("alibaba.item.edit.schema.get", 1);
        System.out.println(JSON.toJSONString(result));
    }
    ...
    public void test_addDocuments() {
        ...
        polarPostgresDB.addDocuments(documents);
    }
}
