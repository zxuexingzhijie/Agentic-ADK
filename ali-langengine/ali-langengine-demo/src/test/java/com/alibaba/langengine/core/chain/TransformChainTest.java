...
public class TransformChainTest {
    ...
    @Test
    public void test_chatglm_run() {
        ChatOpenAI llm = new ChatOpenAI();
        llm.setTemperature(0d);
        llm.setMaxTokens(1024);

        long start = System.currentTimeMillis();
        String question = "请你扮演下开放平台客服的角色，帮助用户解决遇到的问题，并且登记到工单中，最后你必须给我一个很固定的回答\n" +
                "\n" +
                "以下是用户的问题\n" +
                "\n" +
                "'''\n" +
                ...
                "'''\n" +
                "\n" +
                "帮忙总结一下以上用户是什么问题。并且把问题总结到对应的工单中\n" +
                "1、requestId。请求ID\n" +
                "2、api。API英文名称\n" +
                "\n" +
                "最后只要返回以下格式\n" +
        ...
        TransformChain transformChain = new TransformChain();
        transformChain.setInputVariables(Arrays.asList(new String[]{ "question" }));
        transformChain.setOutputVariables(Arrays.asList(new String[]{ "output" }));
        transformChain.setTransform(inputs -> {
            String s = inputs.get("text").toString();
            Map<String, Object> outputs = new HashMap<>();
            outputs.put("output", s);
            return outputs;
        });

        Map<String, Object> inputs = new HashMap<>();
        inputs.put("question", question);
        Map<String, Object> response = transformChain.run(inputs);
        System.out.println("output:" + response.get("output"));
