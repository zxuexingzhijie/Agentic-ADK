...
import static com.alibaba.langengine.mcp.spec.Schema.JSONRPC_VERSION;
...
public class DefaultMcpSession implements McpSession {
		...
		this.connection = this.transport.connect(mono -> mono.doOnNext(message -> {
			if (message instanceof JSONRPCResponse) {
				...
				handleIncomingRequest(request).subscribe(response -> transport.sendMessage(response).subscribe(),
						error -> {
							JSONRPCResponse errorResponse = new JSONRPCResponse(JSONRPC_VERSION, request.getId(),
									null, new JSONRPCError(Schema.ErrorCodes.INTERNAL_ERROR, error.getMessage(), null));
	...
	private Mono<JSONRPCResponse> handleIncomingRequest(JSONRPCRequest request) {
		return Mono.defer(() -> {
			...
			if (handler == null) {
				...
				return Mono.just(new JSONRPCResponse(Schema.JSONRPC_VERSION, request.getId(), null,
						new JSONRPCError(Schema.ErrorCodes.METHOD_NOT_FOUND,
								error.getMessage(), error.getData())));
			}

			return handler.handle(request.getParams())
				.map(result -> new JSONRPCResponse(Schema.JSONRPC_VERSION, request.getId(), result, null))
				.onErrorResume(error -> Mono.just(new JSONRPCResponse(Schema.JSONRPC_VERSION, request.getId(),
						null, new JSONRPCError(Schema.ErrorCodes.INTERNAL_ERROR,
	...
	public <T> Mono<T> sendRequest(String method, Object requestParams, TypeReference<T> typeRef) {
		...
		return Mono.<JSONRPCResponse>create(sink -> {
			...
			JSONRPCRequest jsonrpcRequest = new JSONRPCRequest(Schema.JSONRPC_VERSION, method,
	...
	public Mono<Void> sendNotification(String method, Map<String, Object> params) {
		JSONRPCNotification jsonrpcNotification = new JSONRPCNotification(Schema.JSONRPC_VERSION,
				method, params);
