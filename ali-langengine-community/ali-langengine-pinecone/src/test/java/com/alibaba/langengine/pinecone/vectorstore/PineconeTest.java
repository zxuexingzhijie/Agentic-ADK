...
public class PineconeTest {
    ...
    public void test_similaritySearch() {
        // success
        Pinecone pinecone = new Pinecone("example-index", "default3");
        pinecone.setEmbedding(new FakeEmbeddings());

        List<Document> result = pinecone.similaritySearch("接口taobao.traderates.get没有获取评价信息", 1);
        System.out.println(JSON.toJSONString(result));

        result = pinecone.similaritySearch("alibaba.ascp.logistics.offline.send报错是什么原因？", 1);
        System.out.println(JSON.toJSONString(result));

        result =pinecone.similaritySearch("alibaba.item.edit.schema.get", 1);
        System.out.println(JSON.toJSONString(result));
    }
