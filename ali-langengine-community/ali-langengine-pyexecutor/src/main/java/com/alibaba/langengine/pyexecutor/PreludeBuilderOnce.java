...
/**
 * Builds the Python prelude script for single, oneshot executions.
 * <p>
 * This class generates a self-contained Python script that is executed for each
 * {@code executeOnce} call. The script is responsible for:
 * <ul>
 * <li>Setting up the execution environment based on policies passed via environment variables (e.g., resource limits, import rules).</li>
 * <li>Reading the user's code from stdin.</li>
 * <li>Executing the code, with special handling to capture the value of the last expression.</li>
 * <li>Reporting results (values or errors) back to the Java process via structured JSON messages on stdout.</li>
 * <li>Handling exceptions and timeouts, ensuring the process exits with an appropriate status code and provides traceback information.</li>
 * </ul>
 */
public final class PreludeBuilderOnce {
    private PreludeBuilderOnce() {
    }

    /**
     * Renders the complete Python script as a string.
     *
     * @param p The execution policy containing the rules to be embedded in the script.
     * @return A string containing the Python prelude script.
     */
    public static String render(PyExecutionPolicy p) {
        return ""
                + "import sys, os, json, builtins, ast, traceback\n" // [FIX] Import traceback module
                + "PROTO='[[PYEXEC]]'\n"
                ...
                + "EXTRA_ENV_PRESENT = os.getenv('PY_EXTRA_ENV_PRESENT','0')=='1'\n"
                ...
                + "  if IMPORT_MODE=='WL':\n"
                + "    if root=='os' and EXTRA_ENV_PRESENT: return real_import(name,globals,locals,fromlist,level)\n"
                ...
                + "      if val is not None:\n"
                + "        _r = repr(val)\n"
                + "        print(PROTO+json.dumps({\"type\":\"value\",\"repr\":_r}))\n"
                + "        sys.stdout.flush()\n"
                + "        return\n"
                ...
                + "    _msg=f\"{_cls}: {e}\"\n"
                + "    print(PROTO+json.dumps({\"type\":\"error\",\"error\":_msg}))\n"
                + "    sys.stdout.flush()\n"
                + "    raise\n"
                + "  finally:\n"
                + "    try:\n"
                ...
                + "\n"
                + "try:\n"
                + "  msg = json.loads(sys.stdin.readline() or '{}')\n"
                + "  src = msg.get('code','')\n"
                + "  opt = msg.get('opt') or {}\n"
                + "  if opt.get('cwd'):\n"
                + "    try: os.chdir(opt.get('cwd'))\n"
                + "    except Exception as e:\n"
                + "      print(PROTO+json.dumps({\"type\":\"error\",\"error\":f\"invalid cwd: {e}\"})); sys.stdout.flush()\n"
                ...
                + "  print(PROTO+json.dumps({\"type\":\"error\",\"error\":f\"TimeoutError: {e}\"})); sys.stdout.flush()\n"
                + "  print(PROTO+json.dumps({\"type\":\"_done\",\"exit\":124})); sys.stdout.flush()\n"
                + "  sys.exit(124)\n"
                + "except Exception:\n"
